...
describe Inkind::Factory::Entity::Response do
  describe '.create' do
    context 'with a fixed_value_voucher type' do
      it 'returns a FixedValueVoucher response' do
        expect(described_class.create('fixed_value_vouchers', {})).to be_a Inkind::Entity::Response::FixedValueVoucher
      end
    end

    context 'with a fixed_value_recharge type' do
      it 'returns a FixedValueRecharge response' do
        expect(described_class.create('fixed_value_recharges', {})).to be_a Inkind::Entity::Response::FixedValueRecharge
      end
    end

    context 'with a variable_value_recharge type' do
      it 'returns a VariableValueRecharge response' do
        expect(described_class.create('variable_value_recharges', {})).to be_a Inkind::Entity::Response::VariableValueRecharge
      end
    end
